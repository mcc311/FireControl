...
function onMapClick(e) {
    ...
    new_marker
        .getPopup().on('remove', ()=> {
            const id = new_marker.options.id;
            let $form = document.getElementById(`ship-form-${id}`);
            for(let i = 0; i < $form.length; i++){
...
const shipForm = (marker) =>{
    ...
    if(is_enemy){
        ...
        $form.append(toHTMLSelect(EnemyWeapon, `ship-form-${id}_enemy-weapon1`, '敵軍火力1'));
        $form.append(toHTMLSelect(EnemyWeapon, `ship-form-${id}_enemy-weapon2`, '敵軍火力2'));
    }else{
        $form.append(toHTMLSelect(Ally, `ship-form-${id}_ally`, '我軍艦型'));
        $form.append(toHTMLSelect(AllyWeapon, `ship-form-${id}_ally-weapon1`, '我軍火力1'));
        $form.append(toHTMLSelect(AllyWeapon, `ship-form-${id}_ally-weapon2`, '我軍火力2'));
    }
